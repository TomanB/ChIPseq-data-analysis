##### basic ChIPseq analysis steps in R, using BED-files #####
...
# MYC ChIPseq from K562 cell line -> convervative IDR peaks (received from the ENCODE project database 
# -> https://www.encodeproject.org/files/ENCFF552AMR/)
ChIP_bed <- import("~/ENCFF552AMR.bed", format = "BED", extraCols = extraCols_narrowPeak)

### annotate the peaks
data(TSS.human.GRCh38)
ucsc.hg38.knownGene <- genes(TxDb.Hsapiens.UCSC.hg38.knownGene)

# use biomaRt for annotation with ensembl gene ids
ensembl = useMart("ensembl", dataset="hsapiens_gene_ensembl")

# annotate GRanges -> ensembl gene ids + gene symbols/names
ChIP.anno <- annotatePeakInBatch(ChIP_bed, AnnotationData = ucsc.hg38.knownGene)
ChIP.anno <- addGeneIDs(annotatedPeak = ChIP.anno, orgAnn = "org.Hs.eg.db", feature_id_type = "entrez_id",
                        IDs2Add = c("ensembl","symbol"), mart = ensembl)
...
# plot bargraphs of ChIPseq peaks in assigned genomic regions
...
# density plot of peaks around TSS
...
### subset ChIPseq peaks which are located in promoter regions of reference genome hg38 (e.g. -2000 & +200 bp)
genes <- genes(txdb)
...
ChIP_prom_seq <- getAllPeakSequence(ChIP_prom, genome = Hsapiens)
...
# extract peak sequences and obtain DNAstringset
...
# use Molecular Signatures Datebase genesets to search for enrichments in the ChIPseq data -> 
# indidivual subcategories can focused in the analysis via ,category = "H" == hallmark gene sets etc.
...
motifs <- findMotifFgBg(fg.seq = peak_seq, bg.seq = control_seq, max.motif = 1, enriched.only = TRUE, max.width = 15,
                        both.strand = TRUE)

# refine motif edges
refined_motifs <- lapply(motifs$motifs, function(x){motifRG::refinePWMMotifExtend(motifs = x@match$pattern, seqs = peak_seq,
                                                                                  max.width = 15)})

# plot seqlogo
seqLogo(refined_motifs[[1]]$model$prob)
plotMotifLogo(refined_motifs[[1]]$model$prob, motifName = "", font = "Helvetica-Bold", xlcex = 2.9, ylcex = 2.9, ncex = ,
              xlab = "\nPosition", ylab = "Bits\n")
